"""The investigation workflow, aligned to the HHGOA answer format.

One call per case-pack row:

    answer, audit_case = Investigator().run_case(row)

The flow implements the policy end to end: gather graph evidence,
assess fraud probability, recommend initial actions (citing R-rules),
request evidence when the policy calls for it (simulated, deterministic,
recorded in evidence_requests), re-assess, recommend final actions,
decide case-vs-SAR, stop per section 6, and write the case to the graph
so it becomes memory for the next investigation.

The returned `answer` dict is EXACTLY the submission JSON. The returned
`audit_case` is the internal timeline the dashboard renders.
"""
import time
...
MAX_ROUNDS = 2  # policy s5: verify, then step-up; then decide with what we have
...
class Investigator:
    def __init__(self, graph: GraphClient | None = None):
        ...
        self._tokens = 0  # rough LLM token estimate per case

    # ==================================================================
    # Evidence gathering: the analyst's checklist, automated
    # ==================================================================
    def _gather(self, ctx: dict, row: dict, case: Case) -> tuple[dict, list, dict]:
        """Walk the graph around the flagged transaction and return
        (signals for the assessor, evidence claims with provenance,
        txn lookup of {txn_id: (ts, amount)} for episode/exposure maths)."""
        sig: dict = {"model_score": ctx["risk_score"], "channel": ctx["channel"],
                     "new_device": ctx["device_new"], "proxy": ctx["proxy"]}
        evidence: list = []
        ...
        card_id = ctx["card_id"]
        claim(f"Flagged transaction {ctx['txn_id']}: ${ctx['amount']:.2f}, {ctx['channel']}, "
              f"product {ctx['product_cd']}, region {ctx['addr1']}, model score {ctx['risk_score']:.2f} "
              f"(treated as a reason to look, not a verdict)",
              f"query:case_context(txn_id={ctx['txn_id']})", [ctx["txn_id"], card_id])

        # --- device familiarity (online only) --------------------------
        if ctx["channel"] == "online" and ctx["device_profile"]:
            claim(f"Device profile '{ctx['device_profile']}' is marked "
                  ...
                  f"query:case_context(txn_id={ctx['txn_id']})", [ctx["txn_id"]])

        # --- card window: testing sequences and bursts -----------------
        ...
        if len(small) >= 3:
            ...
            sig["testing_sequence"] = {"small_count": len(small),
                                       "large_amount": big["amount"] if big else None,
                                       "txn_ids": [t["txn_id"] for t in small] + ([big["txn_id"]] if big else [])}
            claim(f"{len(small)} authorisations under $5 within one hour on {card_id}"
                  + (f", followed by a ${big['amount']:.2f} purchase" if big else ""),
                  f"query:card_window(card_id={card_id}, hours=2)", sig["testing_sequence"]["txn_ids"])

        # --- baseline vs the flagged amount / product ------------------
        ...
        if hist["txn_count"] >= 3:
            ...
            if anomalies:
                ...
                claim(f"Out of profile for {card_id}: " + "; ".join(anomalies)
                      + f" (baseline: {hist['txn_count']} txns)",
                      f"query:card_history(card_id={card_id})", [ctx["txn_id"]])
            if ctx["channel"] and ctx["channel"] not in hist["channels"]:
                sig["mixed_channel_anomaly"] = True
                claim(f"Channel shift: card has only ever transacted {'/'.join(hist['channels'])}, "
                      f"this is {ctx['channel']}",
                      f"query:card_history(card_id={card_id})", [ctx["txn_id"]])
        ...
        if burst and sig.get("amount_anomaly"):
            ...
            claim(f"Burst: {len(burst) + 1} out-of-profile online purchases within 48h",
                  f"query:card_window(card_id={card_id}, hours=48)", sig["cnp_burst"])

        # --- out-of-region (card-present) ------------------------------
        if ctx["channel"] == "in_person" and hist["txn_count"] >= 3 and ctx["addr1"] \
            ...
            claim(f"Card-present use in region {ctx['addr1']} with no prior history there; "
                  ...
                  f"query:region_activity(card_id={card_id}, region={ctx['addr1']})", reg["region_txn_ids"])

        # --- recurring-charge check (R7) --------------------------------
        if self.graph.recurring_pattern(card_id, ctx["amount"], ctx["ts"]):
            sig["recurring_match"] = True
            claim(f"Charge matches this card's own recurring rhythm: similar amount roughly monthly",
                  f"query:recurring_pattern(card_id={card_id})", [ctx["txn_id"]])

        # --- shared device across cards (R6 / undocumented) -------------
        shared_cards = []
        if ctx["device_profile"]:
            ...
            if shared_cards:
                ...
                claim(f"Device profile also used by {len(shared_cards)} other card(s) in a 30-day window: "
                      f"{', '.join(shared_cards[:5])}",
                      f"query:device_neighbors(device_profile=...)", shared_cards[:10])
        ...
        for s in sims:
            claim(f"Closed case {s['case_id']} ({s['outcome']}, pattern {s['pattern']}) is similar: "
                  f"{', '.join(s['why_similar'])}. Notes: {s['notes'][:150]}",
                  f"memory:closed_cases(case_id={s['case_id']})", [s["case_id"]], source="document")

        # --- the trigger itself is evidence ------------------------------
        if row.get("trigger_type") == "customer_report":
            sig["customer_response"] = "denied"
            claim("Customer reported the transaction as unrecognised (trigger message)",
                  "case_pack:trigger_text", [ctx["txn_id"]], source="customer")
        return sig, evidence, txn_lookup

    # ==================================================================
    # Recommendations: (assessment, signals) -> actions with R-citations
    # ==================================================================
    def _recommend(self, a: Assessment, sig: dict, exposure: float, stage: str) -> list[dict]:
        ...
        if a.verdict == "fraud":
            ...
            if denied and sig.get("recurring_match"):
                # The R7 special case: disputed but matches their own rhythm.
            ...
            if stage == "initial" and self.policy.r1_verify_first(a.fraud_probability, a.independent_evidence):
                add("VERIFY_WITH_CUSTOMER",
                    f"R1: probability {a.fraud_probability} on a thin signal - verify before any block")
    ...
    # ==================================================================
    # Episode reconstruction: which transactions, how much exposure
    # ==================================================================
    @staticmethod
    ...
    # ==================================================================
    # LLM narrative helpers (grounded; deterministic fallback stays honest)
    # ==================================================================
    def _narrate(self, instruction: str, evidence: list, sims: list, fallback: str) -> str:
        ...
        if out.startswith("[deterministic"):
            return fallback  # no LLM configured: use our structured template
        self._tokens += (len(context) + len(out)) // 4  # rough token estimate
        return out
    ...
    def run_case(self, row: dict) -> tuple[dict, Case]:
        ...
        self._tokens = 0
        case = self.cases.new_case({"type": row.get("trigger_type"), "txn_id": row.get("flagged_txn_id"),
        ...
        if not ctx.get("found"):
            # Honest failure path: never invent IDs (grading rule).
        ...
        # 1. Gather graph evidence + assess.
        ...
        # 2. Initial recommendation - recorded BEFORE any evidence request.
        ...
        # 3. Evidence loop under the section-6 stopping rule.
        ...
        while not stop:
            ...
            if sig.get("customer_response") is None or (sig.get("recurring_match") and rounds == 1
                ...
                # Cheapest decisive evidence first: ask the cardholder.
                # (For an R7 recurring dispute we re-verify with a reminder.)
                resp, assumed = act.simulate_customer_response(ctx["txn_id"], a.fraud_probability)
                sig["customer_response"] = resp
                evidence_requests.append({"type": "customer_validation",
                                          "asked_after_step": step_counter, "assumed_response": assumed})
                evidence.append({"claim": assumed, "source": "customer",
                                 "ref": f"evidence_request:{len(evidence_requests)}", "entity_ids": []})
                ...
                evidence_requests.append({"type": "step_up_auth",
                                          "asked_after_step": step_counter, "assumed_response": assumed})
                evidence.append({"claim": assumed, "source": "customer",
                                 "ref": f"evidence_request:{len(evidence_requests)}", "entity_ids": []})
            ...
            settled = True  # a verification response settles the question (s6)
            stop, stop_reason = should_stop(a, settled, rounds, MAX_ROUNDS)
        ...
        if evidence_requests:
            ...
            if i_set != f_set:
                what_changed = (f"The assumed evidence ({evidence_requests[-1]['type']}) moved probability to "
        ...
        case.risk_level = ("critical" if a.fraud_probability >= 0.85 else "high" if a.fraud_probability >= 0.65
                           else "medium" if a.fraud_probability > 0.30 else "low")

        # 5. SAR - must agree with FILE_REPORT in the final actions.
        ...
        # 6. Case summary (grounded via GraphRAG; template fallback offline).
        fallback_summary = (f"{row.get('trigger_type')} alert on {ctx['txn_id']} (${ctx['amount']:.2f}, {ctx['channel']}). "
        ...
        summary = self._narrate(
            ...
            f"pattern {a.pattern}) an analyst could read.", evidence, sims, fallback_summary)
        case.summary = summary

        # 7. Write the case to the graph: it is now memory.
        graph_case_id = f"CASE-2016-{row['case_id']}"
        self.graph.upsert_case({"graph_case_id": graph_case_id, "case_id": row["case_id"],
                                "customer_id": ctx["customer_id"], "card_id": ctx["card_id"],
                                "pattern": a.pattern, "verdict": a.verdict, "status": status,
                                "fraud_probability": a.fraud_probability,
                                "affected_txn_ids": affected, "connected_card_ids": connected_cards,
                                "summary": summary, "outcome": a.verdict})

        # 8. Assemble the answer file - exactly the submission schema.
        answer = {
            "case_id": row["case_id"],
            "case": {
                "status": status,
        ...
        case.answer = answer  # dashboard convenience (plain attribute)
        return answer, case
    ...
    def _build_sar(self, file_sar: bool, a: Assessment, ctx: dict, row: dict, affected: list,
        ...
        if not file_sar:
            reason = ("No filing: " + ("verdict is not fraud" if a.verdict != "fraud" else
        ...
        reason = ("R2/3a: strongly suspected fraud with "
                  + ("a shared device profile linking additional cards" if shared else f"exposure ${exposure:.2f} over $1,000"))
        # who / what / when / where / how / why - built from evidence, then
        # polished by the LLM when one is configured.
        fallback = (
        ...
        narrative = self._narrate(
            ...
            "how, and why it is suspicious. It must stand on its own.", evidence, sims, fallback)
    ...
    def _empty_answer(row: dict, why: str) -> dict:
        return {"case_id": row["case_id"],
                "case": {"status": "escalated", "verdict": "uncertain", "fraud_probability": 0.5,
                ...
                "evidence_requests": [],
                "next_best_actions": {"initial": [{"action": "ESCALATE_TO_ANALYST", "route": "auto", "reason": "R8"}],
                                        "final": [{"action": "ESCALATE_TO_ANALYST", "route": "auto", "reason": "R8"}],
                                        "what_changed": "nothing"},
                "sar": {"file": False, "reason": why, "narrative": "", "subjects": [],
                         "total_amount_usd": 0, "activity_dates": []},
